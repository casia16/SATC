...
filterScaffold <- function(dat,minLength=1e5,top=5,range=c(0.3,2)){
...
plotGroup <- function(x,col=1:2,...){
  ...
  title(ylab=paste("PC1 ","(",round(sex$pca$d[1]/sum(sex$pca$d)*100, 2),"%)",sep=""), line=3, cex.lab=1.5, family="Calibri Light")
  title(xlab=paste("PC2 ","(",round(sex$pca$d[2]/sum(sex$pca$d)*100, 2),"%)",sep=""), line=3, cex.lab=1.5, family="Calibri Light")
}
...
plotScafs <- function(x,...){
  ...
  par(mar = c(9, 7, 2, 3))
  barplot(rbind(female,male),xlab="",las=2,cex.names=.9,border=NA,col=c("black","red"),beside=T,legend=c("female","male"),ylab="normalized depth",ylim=c(0,1.3),names=x$sexScaf,...)
...
plotScafBoxplot<-function(x,...){
  ...
  ggplot(mydaf2, aes(scaf, depth, fill=factor(sex_lab))) +
    ...
    geom_vline(xintercept = seq(1,sum(x$sexScafsLogic),1)+0.5, linetype="solid", color = "darkgrey", size=0.5)+
...
plotAutosome<-function(x,...){
  mat <- sapply(x$dat,function(y) y$norm)
  autos_list=data.frame(Name=x$dat[[1]]$V1[x$autoScafLogic],Len=x$dat[[1]]$V2[x$autoScafLogic],Mean=rowMeans(mat[x$autoScafLogic,]),SD=apply(mat[x$autoScafLogic,],1, sd, na.rm = TRUE))
...
plotOutlier<-function(x,...){
  mat <- sapply(x$dat,function(y) y$norm)
  outlier_list=data.frame(Name=x$dat[[1]]$V1[x$outlierScafLogic],Len=x$dat[[1]]$V2[x$outlierScafLogic],Mean=rowMeans(mat[x$outlierScafLogic,]),SD=apply(mat[x$outlierScafLogic,],1, sd, na.rm = TRUE))
